...
class BorrowCartActivity : AppCompatActivity() {
                ...
                if (response.isSuccessful) {
                    ...
                    if (userDetails != null) {
                        ...
                        if (!profilePictureUrl.isNullOrEmpty()) {
                            Glide.with(this@BorrowCartActivity)
                                .load(profilePictureUrl)
                                .into(userIcon)
                        }
                    }
                } else {
                    Toast.makeText(this@BorrowCartActivity, "Failed to fetch user details.", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<UserDetailsResponse>, t: Throwable) {
                Toast.makeText(this@BorrowCartActivity, "Network error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
