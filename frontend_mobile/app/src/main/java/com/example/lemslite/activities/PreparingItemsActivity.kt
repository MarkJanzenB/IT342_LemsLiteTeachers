...
class PreparingItemsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_preparing_items)
                ...
                if (response.isSuccessful) {
                    ...
                    if (userDetails != null) {
                        ...
                        if (!profilePictureUrl.isNullOrEmpty()) {
                            Glide.with(this@PreparingItemsActivity)
                                .load(profilePictureUrl)
                                .into(userIcon)
                        }
                    }
                } else {
                    Toast.makeText(this@PreparingItemsActivity, "Failed to fetch user details.", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<UserDetailsResponse>, t: Throwable) {
                Toast.makeText(this@PreparingItemsActivity, "Network error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
